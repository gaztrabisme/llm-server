...
GRADER_PATTERNS = {
    ...
    "gsm8k": r'\b(\d+)\b',
    "gpqa": r'\\boxed{([A-D])}|[Aa]nswer:\s*([A-D])\b',
}
...
""",
    "gpqa": """Answer the following multiple choice question. Think step by step, then put your final answer (A, B, C, or D) inside \\boxed{{}}.
...
class Grader:
    ...
    def _extract_answer_regex(self, pred: str) -> Optional[str]:
        ...
        # For AIME/GPQA datasets, prioritize boxed answers
        if self.dataset_type in ["aime", "aime2025", "gpqa"]:
...
class Processor:
    ...
    async def _make_request_async(
        self, session, server_config: ServerConfig, eval_state: EvalState, prompt: str
    ) -> Tuple[Dict[str, Any], int, Optional[float], Optional[float], str]:
    ...
    async def _process_single_case_async(
        self, session, server_config: ServerConfig, eval_state: EvalState,
        i: int, task_id: str
    ) -> TaskState:
        """Async version of _process_single_case."""
        question_text, prompt, expected = eval_state.get_case(i)
        ...
        try:
            response, tokens, tps_gen, t_gen_ms, finish_reason = await self._make_request_async(
                session, server_config, eval_state, prompt
            )
            result = response["choices"][0]["message"]["content"]
            reasoning_content = response["choices"][0].get("message", {}).get("reasoning_content")
            task_state.response = result
    ...
    async def _evaluate_async(
        self, eval_state: EvalState, concurrency: int,
        verbose: bool = False
    ):
        """Run evaluation with asyncio concurrency using aiohttp and a semaphore."""
        import aiohttp
        ...
        # Use the first server config for all async requests (concurrency mode
        # targets a single server with -np N parallel slots)
        server_config = self.server_configs[0]

        # aiohttp timeout: disable per-request read timeout since LLM generation
        # can take minutes for long thinking chains
        timeout = aiohttp.ClientTimeout(total=None)

        async with aiohttp.ClientSession(timeout=timeout) as session:

            async def run_task(i: int, task_id: str):
                nonlocal n_correct, session_time, start_time
                async with semaphore:
                    task_state = await self._process_single_case_async(
                        session, server_config, eval_state, i, task_id
                    )
            ...
            tasks = [
                asyncio.ensure_future(run_task(i, task_id))
                for i, task_id in eval_state.tasks
            ]

            # gather with return_exceptions=True so one failure doesn't cancel
            # the rest
            results = await asyncio.gather(*tasks, return_exceptions=True)

            # Log any unexpected exceptions that weren't caught inside run_task
            for idx, result in enumerate(results):
                if isinstance(result, Exception):
                    task_i, task_id = eval_state.tasks[idx]
                    print(f"  [async error] {task_id}: {result}", file=sys.stderr)

        print(f"\nSession time: {session_time:.1f}s | Total accumulated time: {eval_state.total_time:.1f}s")
        eval_state.print_summary()
        eval_state.dump()

    def evaluate(self, eval_state: EvalState, verbose: bool = False,
        ...
        if concurrency > 1:
            print(f"Concurrency: {concurrency} (async aiohttp)")
        ...
        # --- async path: use aiohttp + semaphore ---
        if concurrency > 1:
            asyncio.run(self._evaluate_async(
                eval_state, concurrency=concurrency, verbose=verbose
            ))
            return

        # --- synchronous path (unchanged): thread pool + queue ---
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of concurrent async requests (default: 1, synchronous). "
             "Use with llama-server -np N -kvu for N parallel slots."
    )
    ...
    if args.dataset == "gpqa" and args.grader_type not in ("llm", "regex"):
        print("Error: GPQA dataset requires --grader-type llm or regex")
        parser.print_help()
        sys.exit(1)
